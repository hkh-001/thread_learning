...
__constant__ int d_scharrX[3][3] = {
    {-3,  0,  3},
    {-10, 0, 10},
    {-3,  0,  3}
};

__constant__ int d_scharrY[3][3] = {
    {-3, -10, -3},
    { 0,   0,  0},
    { 3,  10,  3}
};
...
__global__ void scharrKernel(const unsigned char* input, float* output, int width, int height) {
    ...
    for (int ki = -1; ki <= 1; ++ki) {
        for (int kj = -1; kj <= 1; ++kj) {
            ...
            gx += pix * static_cast<float>(d_scharrX[ki + 1][kj + 1]);
            gy += pix * static_cast<float>(d_scharrY[ki + 1][kj + 1]);
        }
    }
...
Image applyScharrCUDA(const Image& input) {
    ...
    scharrKernel<<<grid, block>>>(d_input, d_output, input.width, input.height);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Image output = applyScharrCUDA(input);
        auto t2 = std::chrono::high_resolution_clock::now();

        saveImage(output_file, output);

        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
        std::cout << "Scharr CUDA 卷积完成\n";
        std::cout << "耗时: " << ms << " ms\n";
        return 0;
