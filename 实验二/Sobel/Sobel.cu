...
__global__ void sobelKernel(const unsigned char* input, float* output, int width, int height) {
    ...
    for (int ki = -1; ki <= 1; ++ki) {
        for (int kj = -1; kj <= 1; ++kj) {
            ...
            gx += pix * static_cast<float>(d_sobelX[ki + 1][kj + 1]);
            gy += pix * static_cast<float>(d_sobelY[ki + 1][kj + 1]);
        }
    }
...
Image applySobelCUDA(const Image& input) {
    ...
    sobelKernel<<<grid, block>>>(d_input, d_output, input.width, input.height);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Image output = applySobelCUDA(input);
        auto t2 = std::chrono::high_resolution_clock::now();

        saveImage(output_file, output);

        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
        std::cout << "Sobel CUDA 卷积完成\n";
        std::cout << "耗时: " << ms << " ms\n";
        return 0;
