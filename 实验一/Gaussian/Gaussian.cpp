        ...
        for (int j = 0; j < input.width; ++j) {
            ...
            for (int ki = -1; ki <= 1; ++ki) {
                for (int kj = -1; kj <= 1; ++kj) {
                    ...
                    sum += pix * gaussian[ki + 1][kj + 1];
                }
            }
            output[static_cast<size_t>(i) * input.width + j] = sum;
        }
    }
    return nullptr;
}

Image floatToImage(const std::vector<float>& buffer, int width, int height) {
    Image out;
    out.width = width;
    out.height = height;
    ...
    for (size_t i = 0; i < buffer.size(); ++i) {
        int v = static_cast<int>(std::lround(buffer[i]));
        out.data[i] = static_cast<unsigned char>(v < 0 ? 0 : (v > 255 ? 255 : v));
    }
    return out;
}

Image applyGaussianParallel(const Image& input, int num_threads) {
    if (num_threads <= 0) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        ...
        if (pthread_create(&threads[t], nullptr, gaussianWorker, &args[t]) != 0) {
    ...
    return floatToImage(output, input.width, input.height);
}
...
void printUsage(const char* prog) {
    std::cout << "用法:\n"
              ...
              << "示例:\n"
              << "  " << prog << " lena.png out_gaussian.png 4\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Image output = applyGaussianParallel(input, num_threads);
        auto t2 = std::chrono::high_resolution_clock::now();

        saveImage(output_file, output);

        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
        std::cout << "Gaussian 并行卷积完成\n";
