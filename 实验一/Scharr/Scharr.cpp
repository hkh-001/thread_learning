        ...
        for (int j = 0; j < input.width; ++j) {
            ...
            for (int ki = -1; ki <= 1; ++ki) {
                for (int kj = -1; kj <= 1; ++kj) {
                    ...
                    gx += pix * scharrX[ki + 1][kj + 1];
                    gy += pix * scharrY[ki + 1][kj + 1];
                }
            }
...
Image applyScharrParallel(const Image& input, int num_threads) {
    if (num_threads <= 0) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        ...
        if (pthread_create(&threads[t], nullptr, scharrWorker, &args[t]) != 0) {
...
void printUsage(const char* prog) {
    std::cout << "用法:\n"
              ...
              << "示例:\n"
              << "  " << prog << " lena.png out_scharr.png 4\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Image output = applyScharrParallel(input, num_threads);
        auto t2 = std::chrono::high_resolution_clock::now();

        saveImage(output_file, output);

        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
        std::cout << "Scharr 并行卷积完成\n";
